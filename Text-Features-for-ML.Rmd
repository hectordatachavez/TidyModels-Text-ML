...
This projects is inspired by the [Apr 23, 2021 screecast episode](https://www.youtube.com/watch?v=XYj8vyK864Y) of Julia Silge. In that post, Julia walks through how to build features from text for tidymodeling.

I this GitHub repo I aimto build a Supervised Machine Learning Classification Model that can predict the type of whether a item in the Netflix catalog is either a TV-Show or a Movie based on the description and/or the title.
...
```{r original_model_recipe, echo=FALSE, warning=FALSE}
...
```{r model_prep_comb, echo=FALSE, warning=FALSE}

# Set seed for reproducibility
set.seed(1234)
# Define the splits in the data
netflix_split_comb <- netflix_titles %>%
...
# Define the training set
netflix_train_comb <- training(netflix_split_comb)
# Define the testing set
netflix_test_comb <- testing(netflix_split_comb)


# Set seed for reproducibility
set.seed(2345)
# Define the CV folds in the training dataset, stratified again by type
netflix_folds_comb <- vfold_cv(netflix_train_comb, strata = type)
...
# In this model recipe, I'm combining the tokens from description with the sub-word level tokens for predicting type

netflix_rec_comb <- 
  # Define the model and the (training) data
  # Type is explained by the text in description and in title
  recipe(type ~ description + title, data = netflix_train_comb) %>%
  # Define the steps of processing
  # tokenize text in description
  step_tokenize(description) %>%
  # filter tokenlist based on frequency
...
# Define workflow
netflix_wf_comb <- workflow() %>%
  # add recipe
  add_recipe(netflix_rec_comb) %>%
  # add model
  add_model(svm_spec)
